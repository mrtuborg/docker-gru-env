...
const PLUGIN_FIELDS: Record<string, Field[]> = {
  ...
  azure: [
    ...
    { key: 'container',       label: 'Container Name',     type: 'text',   placeholder: 'artifacts', wizard: true,
      hint: (v) => `The blob container to list/access. Run: az storage container list --account-name ${v.storage_account || '<account>'} --auth-mode login --query "[].name" -o tsv` },
    ...
    { key: 'tenant_id',       label: 'Tenant ID',          type: 'text',   placeholder: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', wizard: true,
      hint: 'Your Azure AD directory ID. Run: az account show --query tenantId -o tsv  (or find it in Azure Portal → Azure Active Directory → Overview)',
    ...
    { key: 'client_id',       label: 'Client ID (App ID)', type: 'text',   placeholder: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', wizard: true,
      hint: 'Azure Portal → App registrations → New registration → copy "Application (client) ID". On Authentication tab: add Mobile/desktop platform + enable "Allow public client flows". On API permissions: add Azure Storage → user_impersonation.',
      showWhen: { field: 'auth_method', values: ['azure_ad', 'service_principal'] } },
