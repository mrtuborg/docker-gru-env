...
class IAnalyticsStore(Protocol):
    ...
    async def read_overview(self, days: int = 90) -> dict:
        """Cross-pipeline analytics overview: totals, recent runs, daily activity, per-pipeline breakdown."""
        ...

    async def read_run_detail(self, run_id: str) -> dict:
        """Single pipeline run: run metadata + its run_items, unscoped by pipeline_id."""
        ...
...
class AnalyticsConnector(GruConnector):
    ...
    async def read_overview(self, days: int = 90) -> dict:
        """Cross-pipeline analytics overview for the web dashboard.

        Unscoped by pipeline_id (unlike read_sessions) — aggregates across every
        pipeline sharing this Postgres instance, mirroring the standalone
        Copilot-cost dashboard's "Projects Overview" page.
        """
        if not self._pool:
            return {"analytics_unavailable": True}

        try:
            async with self._pool.acquire() as conn:  # type: ignore[union-attr]
                totals = await conn.fetchrow(
                    """SELECT COUNT(*) AS total_runs,
                              SUM(issues_processed) AS total_processed,
                              SUM(issues_succeeded) AS total_succeeded,
                              SUM(issues_failed)    AS total_failed
                       FROM pipeline_runs"""
                ) or {}

                item_totals = await conn.fetchrow(
                    """SELECT COUNT(*) AS total_items,
                              SUM(CASE WHEN status IN ('success','completed','done') THEN 1 ELSE 0 END) AS items_succeeded,
            ...
            total_items = item_totals.get("total_items") or 0
            items_succeeded = item_totals.get("items_succeeded") or 0
            return {
                "summary": {
                    "total_runs":       int(total_runs),
                    "total_processed":  int(totals.get("total_processed") or 0),
                    "total_succeeded":  int(totals.get("total_succeeded") or 0),
                    "total_failed":     int(totals.get("total_failed") or 0),
                    "total_items":      int(total_items),
                    "items_succeeded":  int(items_succeeded),
                    "items_failed":     int(total_items - items_succeeded),
                    "success_rate":     round(items_succeeded / total_items * 100, 1) if total_items else 0.0,
                    "total_cost_usd":   round(float(item_totals.get("total_cost_usd") or 0), 4),
                    "total_tokens_input":  int(item_totals.get("total_tokens_input") or 0),
                    "total_tokens_output": int(item_totals.get("total_tokens_output") or 0),
                    "avg_duration_s":   round(float(item_totals.get("avg_duration_s") or 0), 1),
                },
                "by_pipeline": [
                    {
                ...
                "daily": [
                    {"day": r["day"], "items": int(r["items"] or 0), "cost_usd": round(float(r["cost_usd"] or 0), 4)}
                    for r in daily
                ],
            }
        except Exception as exc:
            logger.warning("analytics.read_overview: %s", exc)
            return {"analytics_unavailable": True}

    async def read_run_detail(self, run_id: str) -> dict:
        """Single pipeline run (metadata + items), unscoped by pipeline_id."""
        if not self._pool:
            return {"analytics_unavailable": True}
        try:
            async with self._pool.acquire() as conn:  # type: ignore[union-attr]
                ...
                items = [
                    dict(r) for r in await conn.fetch(
                        """SELECT issue_number, issue_repo, issue_title, stage, status,
            ...
            return {
                "run": {
                    **dict(run),
                    "duration_s": round(float(run["duration_s"] or 0), 1),
                },
                "items": items,
            }
        except Exception as exc:
            logger.warning("analytics.read_run_detail: %s", exc)
            return {"analytics_unavailable": True}
