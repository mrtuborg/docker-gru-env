...
- [2026-07-02] **Killing the web-server process inside `gru-analytics-db` kills the whole container** — PID 1 is `gru-analytics-entrypoint.sh`, which runs `python3 gru-analytics-web.py` as a foreground child; `docker exec ... kill <pid>` exits the container, not just the web server. Use `docker start`/`docker restart` at the container level for a hot-restart.
- [2026-07-02] **`gru-db restart` does NOT pick up a newly built image** — it's just `docker stop`+`docker start`, and `do_start()` reuses the existing container unless the container doesn't exist or its port mapping doesn't match `--port`/`--web-port`. To deploy a rebuilt image, `docker rm -f gru-analytics-db` then `./gru-db start --port 9399 --web-port 9398` (the named volume persists data across recreation).
- [2026-07-02] **Two `gru-db` script versions exist** — the main checkout (`docker-gru-env-server`, branch `feature/gru-server`) has an OLDER version without `build` support; the `vladimir-nosenko-analytics-db-web-ui` worktree has a newer one that added it. Always run `gru-db build` from the worktree, not the main checkout.
- [2026-07-02] **A literal `<style>`/`</style>` (or `<script>`/`</script>`) wrapper embedded inside a Python string constant, when that constant is later wrapped AGAIN by the page-shell function, produces double-nested tags that silently break CSS parsing** — browsers treat everything inside `<style>` as raw text until the first `</style>`, so the inner literal text becomes invalid CSS content. Always keep wrapper tags in exactly one place (the shell function), never duplicated in extracted/templated content.
- [2026-07-02] **Reference dashboard's "Confidence" column (exact/low/unknown dot) reflects premium_requests coverage across the aggregated group, NOT cost coverage** — confirmed by finding rows where cost was partially known (rendered with a `~` prefix) while premium was 0% known (still showed `conf-unknown`), and rows with 100% known premium always showed `conf-exact` regardless of session count. The `~$X.XXXX` tilde prefix on aggregate cost cells is a SEPARATE signal: partial cost coverage (some but not all underlying sessions have `cost_usd`) vs. plain `$X` (all known) vs. `—` (none known).
- [2026-07-02] **`git commit -m "$(cat <<'EOF' ... EOF)"` breaks if the heredoc body contains unescaped double quotes** — they terminate the outer `$(...)` double-quoted substitution early ("unexpected EOF" error). Write the message to a temp file and use `git commit -F /path/to/file` instead.
